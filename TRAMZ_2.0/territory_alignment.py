...
def realign(territory):
    ...
    skillSets = sf.query_all(f"SELECT OwnerId,Skill_Set__r.Name \
                               FROM Territory_SkillSet_Mapping__c WHERE Territory__r.Id = '{territoryId}' AND SkillSet_Object__c LIKE '%Account Location%'")

    skillDict = {x['Skill_Set__r']['Name']:x['OwnerId'] for x in skillSets['records']}
    ...
    if len(zipQuery) == 0:
        ...
        def wrong_owner(locations):
            ...
            banking = locations[(locations['Vertical_Market__c'].isin(vertMarkets[0])) & (locations['Security_Owner__c'] != skillDict['Security_Banking_Skill'])]
            banking.Security_Owner__c = skillDict['Security_Banking_Skill']

            education = locations[(locations['Vertical_Market__c'].isin(vertMarkets[1])) & (locations['Security_Owner__c'] != skillDict['Security_Education_Skill'])]
            education.Security_Owner__c = skillDict['Security_Education_Skill']

            slg = locations[(locations['Vertical_Market__c'].isin(vertMarkets[2])) & (locations['Security_Owner__c'] != skillDict['Security_SLG_Skill'])]
            slg.Security_Owner__c = skillDict['Security_SLG_Skill']

            territory = locations[(locations['Vertical_Market__c'].isin(vertMarkets[3])==False) & (locations['Security_Owner__c'] != skillDict['Security_TerritoryRep_Skill'])]
